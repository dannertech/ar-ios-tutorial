...
open class ARViewController: UIViewController, ARTrackingManagerDelegate
{
  ...
  @objc internal func displayTimerTick()
    ...
    fileprivate func loadCamera()
    {
        ...
        //===== View preview layer
      if let cameraLayer : AVCaptureVideoPreviewLayer = AVCaptureVideoPreviewLayer(session: self.cameraSession)
        {
    ...
    open class func createCaptureSession() -> (session: AVCaptureSession?, error: NSError?)
    {
      ...
      let videoDevices = AVCaptureDevice.DiscoverySession(deviceTypes: [AVCaptureDevice.DeviceType.builtInWideAngleCamera, AVCaptureDevice.DeviceType.builtInTelephotoCamera], mediaType: .video, position: .back)
      //devices(for: AVMediaType.video)
        
        // Get back video device
      if let videoDevices : [AVCaptureDevice] = videoDevices.devices
        {
  ...
  @objc internal func debugButtonTap()
    ...
    func addDebugUi()
    {
      ...
      debugLabel.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin]
